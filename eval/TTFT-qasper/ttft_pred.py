# pred_my.py  —— 简单版 LongBench 推理脚本（单卡、支持本地数据）
...
def new_attention_forward(
    ...
    **kwargs,
    ...
    if key_states.shape[2] == query_states.shape[2]:    #在 prefill 阶段
        ...
        ########################################################################################################################
        attn_weights = torch.matmul( #.   decode阶段使用全量的注意力，不使用 xattn 算法
...
def main():
    ...
    # 1. 读 config（dataset2prompt / dataset2maxlen）
    ...
    for name, module in model.named_modules():
        if name.split(".")[-1] == "self_attn":
            ...
            if args.method == "xattn":
                module.threshold = torch.tensor(max[layer_idx])    #给每层传入threshold
