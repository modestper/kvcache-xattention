...
# ⭐ 新增：简单的“只要答案”抽取函数（优先取第一个整数）
def extract_answer(text: str) -> str:
    """
    从模型原始输出中抽取“最终答案”。

    策略：
    1. 优先抽取第一个整数（适合 RULER 里很多数值题）
    2. 如果没有整数，就取第一行的第一个 token
    """
    if not text:
        return ""
...
def main():
    ...
    def get_output(
        ...
        for (
            ...
            # 兼容不同 wrapper：pred["text"] 可能是 str 或 [str]
            if isinstance(pred["text"], str):
                raw_text = pred["text"]
            elif len(pred["text"]) > 0:
                raw_text = pred["text"][0]
            else:
                raw_text = ""

            # ⭐ 关键一步：对原始生成结果做“答案抽取”，只保留最终答案
